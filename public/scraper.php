...
function absUrl($baseURL, $relativePath) {
	if ($relativePath == '') {
...
class Scraper {
	...
	public function __construct($url) {
		...
		foreach (pq('a') as $a) {
			$href = pq($a)->attr('href');
			$url = absUrl($this->_url, $href);
			$dep = false;
			foreach ($this->_hrefs as $href) {
				if ($url === $href['url']) {
					$dep = true;
					break;
				}
...
try {
	...
	$scraper = new Scraper($_GET['url']);

	$result = [
		'status' => true,
		'hrefs' => $scraper->getHrefs(),
	];
